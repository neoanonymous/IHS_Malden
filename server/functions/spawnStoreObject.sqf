...
_isWalStore = ["WalMart", _npcName] call fn_startsWith;
//_isWalStore2 = ["WalMart", _npcName] call fn_startsWith;

private _storeNPC = missionNamespace getVariable [_npcName, objNull];
private _marker = _npcName;

if (_key != "" && _player isKindOf "Man" && {_isGenStore || _isGunStore || _isVehStore || _isWalStore}) then
{
	...
	if (_isGenStore || _isGunStore || _isWalStore) then
	{
		...
		switch (true) do
		{
			...
			//case _isWalStore: { _objectsArray = genObjectsArray };
			//case _isWalStore: { _objectsArray = staticGunsArray };
		};
		
		switch (true) do
		{
			case _isWalStore2: { _objectsArray = staticGunsArray };
		};
	...
	if (_isWalStore) then //WALMART!!!!!!!!  Fractured gaming this is why you dont fire me BITCHES FUCK YOU
	{
		// LAND VEHICLES
		{
