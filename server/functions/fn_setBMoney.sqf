...
// BE VERY CAREFUL - remoteExecutedOwner propagates across calls, so if player A remoteExecs function X to the server, and X calls setbmoney on player B, player A is banned.

// if _absolute is false, _amount will be added/subtracted to the current bmoney value
// if _absolute is true, bmoney will be set directly to _amount
...
private _newAmount = (([_player getVariable ["bmoney",0], 0] select _absolute) + _amount) max 0;

if (isServer) then
{
	if (isRemoteExecuted && remoteExecutedOwner != clientOwner && (remoteExecutedOwner != owner _player || !(_player isKindOf "Man"))) exitWith
	{
		["forged setBMoney", _this] call A3W_fnc_remoteExecIntruder;
	};

	if (alive _player) then
	{
		_player setVariable ["bmoney", _newAmount, true];
	}
	else // we cannot setVariable true if player is dead otherwise it will cause a BattlEye setvariable kick on respawn if "bmoney" is blacklisted
	{
		_player setVariable ["bmoney", _newAmount, false];
		[_player, ["bmoney", _newAmount, false]] remoteExecCall ["setVariable", - clientOwner]; // -2
	};
}
else
{
	...
	_this remoteExecCall ["A3W_fnc_setBMoney", 2];
};
