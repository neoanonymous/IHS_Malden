...
if (_eng) then {
	_vehicle vehicleChat format ["Stop engine in 10s to start resupply.  Vehicles = $250, Military = $500, Planes and Choppers = $1000.  Service will take 30 seconds."];
	sleep 10;
	_eng = isengineon _vehicle;
...
if((player == driver _vehicle) && (!_eng))then {
	...
	if(_type == "B_APC_Tracked_01_CRV_F") then {_price = 5000;};	
	if(_type == "B_APC_Tracked_01_rcws_F") then {_price = 5000;};
	if(_type == "I_APC_tracked_03_cannon_F") then {_price = 5000;};
	...
	if(_type == "B_MBT_01_cannon_F") then {_price = 5000;};
	if(_type == "B_MBT_01_TUSK_F") then {_price = 5000;};
	if(_type == "O_MBT_02_cannon_F") then {_price = 5000;};
	if(_type == "I_MBT_03_cannon_F") then {_price = 5000;};
	...
	if(_type == "I_Heli_light_03_F") then {_price = 1000;};
	if(_type == "B_Heli_Attack_01_F") then {_price = 5000;};
	if(_type == "O_Heli_Attack_02_F") then {_price = 5000;};
	if(_type == "O_Heli_Attack_02_black_F") then {_price = 5000;};
	if(_type == "I_Plane_Fighter_03_AA_F") then {_price = 8000;};
	if(_type == "I_Plane_Fighter_03_CAS_F") then {_price = 8000;};
	if(_type == "B_Plane_CAS_01_F") then {_price = 8000;};
	if(_type == "O_Plane_CAS_02_F") then {_price = 8000;};
	...
	if (_playerMoney < _price) then {
		...
		sleep 2.5;
		_vehicle setFuel 0;
		["Reloading MEMES 2/3...", 2.5] call mf_notify_client;
		sleep 2.5;
		_vehicle setFuel 0;
